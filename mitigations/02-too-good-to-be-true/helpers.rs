...
use anyhow::Result;
// Removed the unused 'rand::seq::IndexedRandom' import to prevent warnings
use uuid::Uuid;
...
pub fn select_downlink_gateway(
    ...
    // ========================================================================
    // ISOLATED SECURITY CHECK: "Too-Good-To-Be-True" Heuristic
    // ========================================================================
    ...
    rx_info.items.retain(|item| {
        ...
        // ---> NEW: Print alert if the Too-Good-To-Be-True heuristic fails
        if !passes_heuristic {
            ...
            println!(
                "\n [SECURITY ALERT] Improbable signal detected from Gateway {}! ", 
                gw_hex
            );
            println!(
                "   -> Reason: Too-Good-To-Be-True heuristic failed (RSSI: {}, SNR: {}). Dropping gateway from candidate pool...\n", 
                item.rssi, item.lora_snr
            );
        }

        // Return whether the gateway passed the test to keep/drop it
        passes_heuristic
    });
    // ========================================================================
    ...
    // Return the first item from the new_items slice
            ...
            Test {
                ...
                rx_info: internal::DeviceGatewayRxInfo {
                    ...
                    items: vec![internal::DeviceGatewayRxInfoItem {
                        lora_snr: -5.0,
                        rssi: -100, // Added default plausible RSSI for the test
